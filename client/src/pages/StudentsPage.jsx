...
const StudentsPage = () => {
  ...
  const printStudentsBasedOnRegistrationDate = async () => {
    try {
      const studentsWithExpiringGreenPass = students
				.filter(({ RegistrationDate, GreenPassExpirationDate, IsAdult }) => {
					if (RegistrationDate) {
						const RegistrationDateFormatted = new Date(RegistrationDate)

						return RegistrationDateFormatted.getMonth() === selectedMonth
							&& RegistrationDateFormatted.getFullYear() === selectedYearGreenPass
							&& IsAdult === ages[0].age && !!GreenPassExpirationDate
					}
    ...
    try {
      const studentsWithExpiringGreenPass =  students.filter(({ GreenPassExpirationDate }) => {
        if (GreenPassExpirationDate) {
          ...
          return GreenPassExpirationDateFormatted.getMonth() === selectedMonth
      ...
      if (studentsWithExpiringGreenPass.length > 0) {
        ...
        pdfMake.createPdf(documentDefinition).open();
			...
			return toast.error(
				`Nessuna allieva con Scadenza Green Pass nel ${month} ${selectedYearGreenPass}`, toastConfig);
    } catch (err) {
			return toast.error('Si e` verificato un errore nel creare il documento', toastConfig);
    }
  }
        ...
        <div className="filter-form">
          ...
          <Form.Group>
            ...
            <Form.Control
							...
							onChange={(e) => { filterColumn('Name', e.target.value) }}/>
          </Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Control
							...
							onChange={(e) => { filterColumn('Surname', e.target.value) }}/>
          </Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Control
							...
							type="text"
							placeholder="Città..."
							onChange={(e) => { filterColumn('City', e.target.value) }}/>
          </Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Control
							ref={filterAgeRef}
							as="select"
							onChange={({ target }) => { filterColumn('IsAdult', target.value) } }
						>
        ...
        <div className="form-wrapper green-pass-form">
          <Form.Group>
            ...
            <Form.Control
							ref={filterAgeRef}
							as="select"
							onChange={({ target }) => { setselectedMonth(parseInt(target.value, 10)) } }
						>
							{
								months.map((month) =>
									<option key={`select_${month.id}`} value={month.id}> {month.month} </option>
								)
							}
						</Form.Control>
          </Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Control
							ref={filterAgeRef}
							as="select"
							onChange={({ target }) => { setSelectedYearGreenPass(parseInt(target.value, 10)) } }
						>
							{
								years.map((year) =>
									<option key={`select_${year.id}`} value={year.id}> {year.year} </option>
								)
							}
						</Form.Control>
          </Form.Group>
